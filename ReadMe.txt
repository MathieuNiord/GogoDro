...
Sujet : Nous voulons modéliser un langage pour contrôler un robot capable de dessiner dans une zone à dessin.
Contraintes : Actions prédéfinies, syntaxe et complexité des mécanismes accessibles à un niveau "collège".
...
SYNTAXE :
    ...
    Soit un axe p(x, y) tq p.x = 0 et p.y = 0 :
    ...
    La syntaxe actuelle ne permet pas de réaliser un tracer diagonal directement. Par conséquent, il faudra
    ...
    - Instructions :
        ...
        Ecrire                                  : Baisse le crayon.
        Couleur(<color>)                        : Change la couleur actuelle en color.
        Centrer                                 : Repositionne le robot, sans écriture, au centre de l'image.
        <variable> <- <expression>              : Affectation, c'est une instruction qui appartient dans l'arithmétique du langage.
        ...
        Deplacer(<direction>, <n>)              : Déplace le robot vers la direction fournie de n pixels.
        Ligne(<direction>, <taille>)            : Dessine une ligne de longueur taille vers la direction fournie.
    ...
    - BONUS :
        ...
        le dernier nom devra exister au préalable. Le chemin à fournir doit être exprimer comme tel (entre des slashs) :
...
AMELIORATIONS :
    ...
    - Ajout de couleurs : puisque les couleurs sont à indiquer uniquement à l'aide d'un identifiant, nous avons ajouté plus de possibilités ;
    - Instructions Centrer et Cercle ;
    - Les variables et expressions sont traitées telles que des double dans l'objectif de pouvoir améliorer la précision du curseur ;
